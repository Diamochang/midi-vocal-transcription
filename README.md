...
## Python 问题
请将能够反映问题的输出截屏或将带有输出内容的笔记本直接保存在本地，然后打开一个新的 Issue，详细说明故障及复现步骤，并提供上述输出。如果你为了解决该故障修改了笔记本内容，那么需要点一次“代码执行程序”下的“断开连接并删除运行时”，重置远程资源后再点击“全部运行”，如仍有故障就把输出截屏或保存，发表 Issue 时一起提供。
## Colab 问题
...
## Dependencies（附仓库依赖调整）

- OS: LINUX 
- Programming language: Python 3.6+ *注：Colab 使用 3.10，而经过调整的依赖只能在版本`>=3.9,<3.11`的条件下才能正常安装。
- Python Library 
  - Keras 2.7.0 (Deep Learning library) *注：为方便在线运行，依赖文件中调整为 2.8.0。
  - tensorflow 2.5.0 (Deep Learning library) *注：为方便在线运行，依赖文件中调整为 2.8.0。
